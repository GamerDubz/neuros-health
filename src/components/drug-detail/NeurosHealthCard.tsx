...
export default function NeurosHealthCard({ drug }: { drug: DrugDetail }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Only render BigThreeRow if there are items */}
      {(drug.big3 || []).length > 0 && <BigThreeRow items={drug.big3} />}

      {/* Only render vibe summary if there's text */}
      {drug.vibe_summary?.trim() && <VibeSummary text={drug.vibe_summary} />}

      <div className="space-y-3">
        {/* Each accordion is self-guarding — returns null if empty */}
      ...
      {/* Red zone only shows when there's real content */}
